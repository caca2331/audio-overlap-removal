...
- 单块对齐失败时自动扩大参考窗重试，彻底失败则原样透传，不中断整个任务；
- 250 ms 对齐锚点按 offset 轨迹隐含的播放速率测量，锁到音乐一个周期之外的
  锚点会被剔除，稳态变速和播放端 EQ 不再损失消除量；
...
- 内存：默认 `--workers 4` 时 4 GB，与输入时长无关，详见
  [性能建议](#性能建议)。
...
2. **分段跟踪与重获取**：局部跟踪失败后限频执行全局搜索，处理暂停、跳转
   和重放；重获成功后回头补测等待期间跳过的步长，让重放从真正的起点开始；
   每个分段保留实测的锚点轨迹，长分段不再被压缩成单一 offset 和斜率；
3. **Offset momentum**：正式消除之前，先在 4 kHz、较宽窗口内逐块复测
   offset，中值滤波成连续轨迹；探测不可靠的块直接继承邻块结果，而不是自
   行猜测；
4. **局部时间对齐**：块内搜索以该预测为中心展开，而非在整个窗口里取最大
   相关；250 ms 锚点按 offset 轨迹隐含的播放速率测量，稳态变速不会把它们
   抹糊；偏离邻居的锚点在 offset 直线附近重测或丢弃；随后用锚点构造时间
   扭曲，必要时验证更密集的候选路径；
...
7. **逐块消除后验证**：由相关分数、momentum 探测和"实际消掉多少能量"三者
   ...
   透传；
8. **分块写出**：按时间顺序写出，相邻两个已消除的块在共享的 context 内交叉
   淡化，结束时汇总所有低置信度区间。
...
`scan` 一项取决于实际走哪条搜索路径，见 [`--scan-mode`](#扫描模式)：
...
默认 `--workers 4` 时峰值**任何时长都约 3.4 GB，4 GB 即够**——峰值由消除
阶段决定，两种扫描都达不到。内存紧张时优先调小 `--workers`，超过 4 之后
也换不来多少速度。
...
- `auto`（默认）——用全相关，任一输入超过四小时时切换到索引；
- `correlation`——比对完整解码波形。对短匹配更敏感，也是唯一能发现「跳到
  参考远处、只播几十秒」这类重放的路径，但耗时随时长平方增长：1 小时约
  90 秒，3 小时约 13 分钟；
- `fingerprint`——查询流式紧凑索引。两小时量级快约 30 倍、内存平坦，代价
  是对短匹配的敏感度下降。

参考媒体是长段连续播放、而扫描成为瓶颈时，强制 `fingerprint`；混音在参考
的不同位置之间反复跳转、每个短匹配都要抓到时，强制 `correlation`。
