...
`0.1 + 0.1 * 小时数`——更长的输入会切换到流式索引，所以 24 小时的文件并
不比 4 小时的更吃内存。`--chunk` 和 `--sample-rate` 按比例影响 worker 那
一项。

默认 `--workers 4` 时，**任何时长都约为 3.4 GB，4 GB 即够**。内存紧张时
优先调小 `--workers`，超过 4 之后也换不来多少速度。
